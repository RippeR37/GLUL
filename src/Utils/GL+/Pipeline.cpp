...
namespace GL {
    ...
    void Pipeline::setCamera(const Util::Interface::Camera* camera) {
        _camera = camera;
    }
    ...
    void Pipeline::setView(const Util::Interface::Camera* camera) {
        setCamera(camera);
    ...
    const glm::mat4& Pipeline::getView() const throw(Util::Exception::FatalError) {
        if(_viewSource == ViewSource::Camera)
            if(_camera != nullptr)
                return _camera->getMatrix();
            else
                throw Util::Exception::FatalError("Request for view matrix without camera");
        else
            return _view;
    }
    ...
    const Util::Interface::Camera* Pipeline::getCamera() const {
        return _camera;
    }
